...
async function renderCatalog() {
  ...
  app.innerHTML = shell(`<section class="page-header"><div><h1>สินค้าที่เปิดรับจอง</h1><p>เลือกสินค้าและจำนวนก่อนส่งคำขอ</p></div><button class="btn btn-outline branch-orders-action" data-page="orders">รายการจองของฉัน</button></section><section class="toolbar"><label class="field">ค้นหาสินค้า<input id="catalog-search" placeholder="รหัส Part หรือชื่อสินค้า" /></label></section><section class="product-list" id="catalog-body"></section><section class="cart" id="cart"></section>`);
  const draw = () => { const search = document.querySelector("#catalog-search").value.toLowerCase(); const rows = items.filter((item) => `${item.id} ${item.name}`.toLowerCase().includes(search)); document.querySelector("#catalog-body").innerHTML = rows.length ? rows.map((item) => { const qty = state.cart.get(item.id)?.quantity || 0; return `<article class="product-card"><div><span class="part">${escapeHtml(item.id)}</span><h2>${escapeHtml(item.name)}</h2></div><div class="product-meta"><span>${escapeHtml(item.round_name)}</span><b>${money.format(item.sell_price)}</b></div><div class="product-quantity"><span>จำนวน</span><span class="qty"><button data-qty="-1" data-part="${escapeHtml(item.id)}" aria-label="ลดจำนวน ${escapeHtml(item.name)}">−</button><strong>${qty}</strong><button data-qty="1" data-part="${escapeHtml(item.id)}" aria-label="เพิ่มจำนวน ${escapeHtml(item.name)}">+</button></span></div></article>`; }).join("") : `<section class="panel empty">ไม่มีสินค้าที่เปิดรับจอง</section>`; document.querySelectorAll("[data-qty]").forEach((button) => button.onclick = () => changeCart(button.dataset.part, Number(button.dataset.qty))); drawCart(); };
...
async function renderOrders() {
  ...
  app.innerHTML = shell(`<section class="page-header"><div><h1>รายการจองของฉัน</h1><p>ติดตามการพิจารณาและการจัดสรรสินค้า</p></div><button class="btn btn-primary" data-page="catalog">สร้างใบจอง</button></section>${orders.length ? `<section class="order-list">${orders.map((order) => `<article class="order-card"><div class="order-card-head"><b>${escapeHtml(order.id)}</b>${badge(order.status)}</div><h2>${escapeHtml(order.round_name)}</h2><div class="order-meta"><div><span>จำนวนที่จอง</span><b>${order.requested_total} ชิ้น</b></div><div><span>จำนวนที่จัดสรร</span><b>${order.status === "PENDING" ? "รอพิจารณา" : `${order.allocated_total} ชิ้น`}</b></div><div><span>วันที่ส่ง</span><b>${fmtDate(order.created_at)}</b></div></div></article>`).join("")}</section>` : `<section class="panel empty">ยังไม่มีรายการจอง</section>`}`);
}
