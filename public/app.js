...
async function allocationDialog(orderId) {
  try {
    ...
    modal.querySelector("#allocation-form").onsubmit = async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const itemsPayload = allocationInputs.map((input) => ({ itemId: input.dataset.item, allocatedQuantity: Number(input.value), note: "" })); try { const result = await api(`/api/admin/orders/${encodeURIComponent(orderId)}/allocation`, { method: "PATCH", body: JSON.stringify({ items: itemsPayload, adminNote: form.get("adminNote") }) }); modal.remove(); await renderAdminOrders(); showMessage(`บันทึกการจัดสรรแล้ว สถานะ: ${statusLabel(result.status)}`); } catch (error) { showMessage(error.message, true); } };
    modal.querySelector("#delete-admin-order")?.addEventListener("click", async () => { if (!confirm("ยืนยันลบคำขอนี้? เมื่อลบแล้วจะกู้คืนไม่ได้")) return; try { await api(`/api/admin/orders/${encodeURIComponent(orderId)}`, { method: "DELETE" }); modal.remove(); await renderAdminOrders(); showMessage("ลบคำขอแล้ว"); } catch (error) { showMessage(error.message, true); } });
