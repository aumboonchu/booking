...
async function renderOrders() {
  ...
  app.innerHTML = shell(`<section class="page-header"><div><h1>รายการความต้องการของฉัน</h1><p>ติดตามจำนวนที่ขอ จำนวนที่จัดสรร และเวลาส่งสินค้า</p></div><button class="btn btn-primary" data-page="catalog">+ สร้างรายการใหม่</button></section>${orders.length ? `<section class="order-list">${orders.map((order) => `<article class="order-card"><div class="order-card-head"><b>${escapeHtml(order.id)}</b>${badge(order.status)}</div><h2>${escapeHtml(order.customer_name)}</h2><p class="order-summary">${escapeHtml(order.item_summary)}</p><div class="order-meta"><div><span>ต้องการ</span><b>${order.requested_total} เครื่อง</b></div><div><span>จัดสรรแล้ว</span><b>${order.allocated_total} เครื่อง</b></div><div><span>แจ้งเมื่อ</span><b>${fmtDate(order.created_at)}</b></div>${order.sent_at ? `<div><span>ส่งสินค้าเมื่อ</span><b>${fmtDate(order.sent_at)}</b></div>` : ""}</div>${order.status === "PENDING" ? `<div class="order-actions"><button class="btn btn-outline btn-small" data-edit-order="${escapeHtml(order.id)}">แก้ไข</button><button class="btn btn-danger btn-small" data-cancel-order="${escapeHtml(order.id)}">ยกเลิก</button></div>` : ""}</article>`).join("")}</section>` : `<section class="panel empty">ยังไม่มีรายการความต้องการ</section>`}`);
